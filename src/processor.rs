...
pub fn add_student_intro(
    ...
    create_reply_counter(program_id, accounts, pda)?;
...
pub fn add_reply(program_id: &Pubkey, accounts: &[AccountInfo], reply: String) -> ProgramResult {
    ...
    let mut counter_data = ReplyCounter::try_from_slice(&reply_counter.data.borrow())?;
    ...
    let mut reply_data = Reply::try_from_slice(&reply_account.data.borrow())?;
...
fn create_reply_counter(
    ...
    let reply_counter = next_account_info(account_info_iterator)?;
    let system_program = next_account_info(account_info_iterator)?;
